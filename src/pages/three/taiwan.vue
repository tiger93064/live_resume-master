<template>
  <div ref="3world" style="width: 100%; height: 600px"></div>
</template>
...
import taiwanGeoInfo from '../../../../configs/taiwan_geo.json'
...
export default {
  ...
  computed: {
    // ...mapState('app', ['legislatorSourceArray']),
    ...mapGetters({
      ntbpServerUrl: ["getNtbpServerUrl"],
    }),
  ...
  methods: {
    ...
    initWorld(){
      ...
      renderer.domElement.addEventListener("mousemove", event => {
        ...
        vm.info.hover = intersection.find(item => item.object)?.object?.name
        vm.$emit('input', vm.info)
        vm.$emit('hover', {x: event.offsetX, y: event.offsetY})
      ...
      renderer.domElement.addEventListener("click", event => {
        ...
        const intersection = raycaster.intersectObjects(scene.children) // 获取射线发射器捕获的模型列表，传进去场景中所以模型，穿透的会返回我们
        console.log(intersection.find(item => item.object)?.object?.name)
        vm.info.selected = intersection.find(item => item.object)?.object?.name
        vm.$emit('input', vm.info)
      })
      ...
      function domResize() {
        ...
        console.log(vm.$refs['3world'].offsetWidth)

        camera.aspect = vm.$refs['3world'].offsetWidth / vm.$refs['3world'].offsetHeight;
        camera.updateProjectionMatrix();

        renderer.setSize( vm.$refs['3world'].offsetWidth, vm.$refs['3world'].offsetHeight );
      ...
      let animate = () => {
        ...
        if(!isSet){
          ...
          // console.log(centerP)
          camera.position.set(centerP.x + 600 , centerP.y - 600, centerP.z + 5000)
  ...
  async mounted() {
    // var l = []
    this.geoInfo = taiwanGeoInfo.features
    this.initWorld()
    this.run()
...
$bg : var(--v-background-base);
$pr : var(--v-primary-base);
...
.primary-elevation {
   animation: mymovea 1s infinite;
}

@keyframes mymovea {
  50% {
    box-shadow: 0 0 40px $pr;
  }
}
